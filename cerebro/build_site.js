...
// El sitio en vivo lee la hoja publicada; esto solo actualiza el fallback
// para cuando el visitante no tiene conexión.
...
fetch(src[1]).then(r => r.text()).then(txt => {
  ...
  console.log(`index.html: instantánea refrescada con ${data.length} películas.`);
});
