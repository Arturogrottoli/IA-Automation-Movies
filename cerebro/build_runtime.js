// Backfill de la duración (minutos) desde TMDB -> ../runtime.json
// Usa el id de TMDB ya resuelto en posters.json (no vuelve a buscar por título).
//   node cerebro/build_runtime.js
const fs = require("fs");
...
(async () => {
  ...
  const outPath = path.join(ROOT, "runtime.json");
  ...
  console.log(`\nruntime.json: ${Object.keys(out).length} entradas (${fetched} nuevas esta corrida, ${sinDato} sin dato)`);
})();
